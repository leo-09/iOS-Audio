...
- (void)createMainBtnWithFrame:(CGRect)frame {
    ...
    _mainBtn.layer.borderColor  = UIColorFromRGB(CTXThemeColor).CGColor;
    _mainBtn.layer.borderWidth  = 1;

    [self addSubview:_mainBtn];
    
    UIView *rightView = [[UIView alloc] initWithFrame:CGRectMake(_mainBtn.frame.size.width - 20, 0, 20, _mainBtn.frame.size.height)];
    rightView.backgroundColor = UIColorFromRGB(CTXThemeColor);
    [_mainBtn addSubview:rightView];
    
    // 旋转尖头
    _arrowMark = [[UIImageView alloc] initWithFrame:CGRectMake(_mainBtn.frame.size.width - 16, 0, 12, 12)];
    ...
    _arrowMark.contentMode = UIViewContentModeScaleAspectFit;
    [_mainBtn addSubview:_arrowMark];
}

- (void)setMenuTitles:(NSArray *)titlesArr rowHeight:(CGFloat)rowHeight superView:(UIView *)view convertRect:(CGRect)rect {
    if (self == nil || !titlesArr) {
        return;
    }
    ...
    _listView.frame = CGRectMake(convertRect.origin.x , convertRect.origin.y + self.frame.size.height, VIEW_WIDTH(self),  0);
    _listView.clipsToBounds       = YES;
    _listView.layer.masksToBounds = NO;
    _listView.layer.borderColor   = UIColorFromRGB(CTXThemeColor).CGColor;
...
//    [self.superview addSubview:_listView]; // 将下拉视图添加到控件的俯视图上
    [superView addSubview:_listView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        cell.textLabel.textColor = UIColorFromRGB(CTXBaseFontColor);
