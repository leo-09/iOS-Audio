...
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame numberOfStars:DEFALUT_STAR_NUMBER  photostr:@"黄"] ;
}
...
- (instancetype)initWithFrame:(CGRect)frame numberOfStars:(NSInteger)numberOfStars photostr:(NSString *)phoneStr {
    if (self = [super initWithFrame:frame]) {
        _numberOfStars = numberOfStars;
        self.photoColor = phoneStr;
        [self buildDataAndUI];
    }
    return self;
}
...
- (void)buildDataAndUI {
    ...
    if ([self.photoColor isEqualToString:@"hong"]) {
    self.foregroundStarView = [self createStarViewWithImage:@"DB_statSelect"];
    self.backgroundStarView = [self createStarViewWithImage:@"DB_star"];
