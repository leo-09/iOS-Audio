...
@interface CCollectionRecordView()

@property (nonatomic, strong) CCollectionRecordCell *tempCell;
@property (nonatomic, retain) NSIndexPath *selectIndexPath;;  // 选择的位置
@property (nonatomic, retain) NSIndexPath *deleteIndexPath;;  // 删除的位置

@end

@implementation CCollectionRecordView
...
- (instancetype) init {
    if (self = [super init]) {
        ...
        [self.tableView registerClass:[CCollectionRecordCell class] forCellReuseIdentifier:@"CCollectionRecordCell"];
        // 刷新header
        ...
        // 创建cell
        self.tempCell = [[CCollectionRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"CCollectionRecordCell"];
    }
...
- (void) updateSelectModel:(CarFriendCardModel *)model {
    if (self.selectIndexPath) {
        CarFriendCommentModel *selectModel = _dataSource[self.selectIndexPath.row];
        selectModel.isLaud = model.isLaud;                      // 是否点赞
        selectModel.cmsCard.laudCount = model.laudCount;
        selectModel.cmsCard.commandCount = model.commandCount;
        selectModel.cmsCard.isRecommend = model.isRecommend;    // 是否推荐
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CCollectionRecordCell *cell = [CCollectionRecordCell cellWithTableView:tableView];
