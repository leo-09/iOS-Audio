...
@implementation CSystemMessageView
...
- (instancetype) init {
    if (self = [super init]) {
        ...
        [self.tableView registerClass:[CSystemMessageCell class] forCellReuseIdentifier:@"CSystemMessageCell"];
        // 刷新header
        self.tableView.mj_header = [CTXRefreshGifHeader headerWithRefreshingBlock:^{
            if (self.refreshSysMsgDataListener) {
                self.refreshSysMsgDataListener(NO);
            }
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CSystemMessageCell *cell = [CSystemMessageCell cellWithTableView:tableView];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.selectSysMsgCellListener) {
        self.selectSysMsgCellListener(self.dataSource[indexPath.row]);
    }
}
...
- (void) addFooter {
    if (!self.tableView.mj_footer) {
        self.tableView.mj_footer = [CTXRefreshGifFooter footerWithRefreshingBlock:^{
            if (self.loadSysMsgDataListener) {
                self.loadSysMsgDataListener();
            }
        }];
    }
}
...
- (void) refreshDataSource:(NSArray *)data {
    ...
    if (_dataSource.count == 0) {
        [self addNilDataView];
        [promptView setNilDataWithImagePath:@"zw_xx" tint:@"暂无平台公告信息" btnTitle:nil];
    } else {
        if (promptView) {
...
- (void) addNilDataView {
    if (!promptView) {
        ...
        [promptView setPromptRefreshListener:^{
            @strongify(self)
            if (self.refreshSysMsgDataListener) {
                self.refreshSysMsgDataListener(YES);
            }
        }];
    }
