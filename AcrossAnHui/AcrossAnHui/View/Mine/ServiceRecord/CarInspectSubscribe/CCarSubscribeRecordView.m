...
@interface CCarSubscribeRecordView()

@property (nonatomic, strong) CCarSubscribeRecordCell *tempCell;

@end

@implementation CCarSubscribeRecordView
...
- (instancetype) init {
    if (self = [super init]) {
        ...
        [self.tableView registerClass:[CCarSubscribeRecordCell class] forCellReuseIdentifier:@"CCarSubscribeRecordCell"];
        // 刷新header
        ...
        // 创建cell
        self.tempCell = [[CCarSubscribeRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"CCarSubscribeRecordCell"];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CCarSubscribeRecordCell *cell = [CCarSubscribeRecordCell cellWithTableView:tableView];
    ...
    [cell setEvaluateSubscribeRecordListener:^(id result) {
        if (self.evaluateSubscribeRecordListener) {
            self.evaluateSubscribeRecordListener(result);
        }
    }];
    [cell setShowEvaluateSubscribeRecordListener:^(id result) {
        if (self.showEvaluateSubscribeRecordListener) {
            self.showEvaluateSubscribeRecordListener(result);
        }
    }];
...
- (void) refreshDataSource:(NSArray *)data {
    ...
    if (_dataSource.count == 0) {
        [self addNilDataView];
        [promptView setNilDataWithImagePath:@"park_zwtcjl" tint:@"暂时没有记录，快去预约吧!" btnTitle:@"预约车检" isNeedAddData:NO];
    } else {
        if (promptView) {
...
- (void) addNilDataView {
    if (!promptView) {
        ...
        [promptView setPromptOperationListener:^ {
            @strongify(self)
            if (self.toInspectSubscribeListener) {
                self.toInspectSubscribeListener();
            }
        }];
    }
