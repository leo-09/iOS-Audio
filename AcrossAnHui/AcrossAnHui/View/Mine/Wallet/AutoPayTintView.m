...
@implementation AutoPayTintView
...
- (void) addItemView {
    UIImage *image = [UIImage imageNamed:@"icon_tk"];
    ...
    [cancelBtn addTarget:self action:@selector(hideView) forControlEvents:UIControlEventTouchDown];
    ...
    [sureBtn addTarget:self action:@selector(openAutoPay) forControlEvents:UIControlEventTouchDown];
    ...
    updateContentLabel.text = @"打开自动支付后，系统将从用户账户中自动扣除绑定车辆的停车费用以及前期的停车欠费。";
    [_bgView addSubview:updateContentLabel];
    [updateContentLabel makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(@24);
        make.right.equalTo(@(-24));
        make.bottom.equalTo(cancelBtn.top).offset(-20);
    }];
}

- (void) openAutoPay {
    if (self.openAutoPayListener) {
        [self hideAnimation];
        self.openAutoPayListener();
    }
}
...
- (void) hideView {
    if (self.cancelAutoPayListener) {
        [self hideAnimation];
        self.cancelAutoPayListener();
    }
}
...
- (void) showAnimation {
    ...
    animation.toValue = [NSValue valueWithCGPoint:CGPointMake(appDelegate.window.center.x, imageHeight / 2)];
