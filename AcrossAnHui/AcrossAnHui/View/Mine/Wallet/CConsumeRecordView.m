...
@implementation CConsumeRecordView
...
- (instancetype) init {
    if (self = [super init]) {
        ...
        [self.tableView registerClass:[CConsumeRecordCell class] forCellReuseIdentifier:@"CConsumeRecordCell"];
        // 刷新header
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ConsumeRecordCollection *collection = _dataSource[section];
    return collection.records.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CConsumeRecordCell *cell = [CConsumeRecordCell cellWithTableView:tableView];
    
    ConsumeRecordCollection *collection = _dataSource[indexPath.section];
    ConsumeRecordModel *record = collection.records[indexPath.row];
...
- (void) addDataSource:(NSArray<ConsumeRecordModel *> *)records {
    // 添加消费记录，并分组
    _dataSource = [ConsumeRecordCollection recordCollection:_dataSource recordModels:records];
...
- (void) refreshDataSource:(NSArray<ConsumeRecordModel *> *)records {
    ...
    if (_dataSource.count == 0) {
        [self addNilDataView];
        [promptView setNilDataWithImagePath:@"park_zwtcjl" tint:@"暂无消费记录" btnTitle:nil];
    } else {
        if (promptView) {
