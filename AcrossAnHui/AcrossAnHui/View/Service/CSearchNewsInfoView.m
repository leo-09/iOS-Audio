...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGSize size = [hotKeyWord textSizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
    return CGSizeMake(size.width + 15, 50); // 注意：每个cell的宽左右空留15，高是50
}
...
- (CGFloat) caculateCollectionViewHeight {
    ...
    for (NSString *keyWord in self.hotKeywords) {
        ...
        CGFloat width = [keyWord textSizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(MAXFLOAT, MAXFLOAT)].width + 15;
        totalWidth += (width + 10);
    }
