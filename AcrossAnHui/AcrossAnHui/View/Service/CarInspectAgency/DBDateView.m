...
@property (nonatomic, copy)NSString *hourStr;//时
@property (nonatomic, copy)NSString *minStr;//分

@end

@implementation DBDateView
...
-(void)setDateDic:(NSArray *)dateDic{
    _selectTime = 0;
    _selectYear = 0;
    _dateDic = dateDic;
...
-(void)setupUI
{
    ...
    self.dateView=[[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight-280, ScreenWidth, 250+10+10+10)];
    ...
    self.chooseTimeLabel=[[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.dateView.size.width, 40)];
    ...
    self.line1=[[UIView alloc] initWithFrame:CGRectMake(0, 39, self.dateView.size.width, 1)];
    self.line1.backgroundColor= RGBA(237, 237, 237, 1.0);
    [self.dateView addSubview:self.line1];
    
    UIView * vieww = [[UIView alloc]initWithFrame:CGRectMake(0, 40+70, self.dateView.size.width, 40)];
    vieww.backgroundColor = RGBA(180, 237, 252, 1.0);
    [self.dateView addSubview:vieww];
    self.datePicker.backgroundColor = [UIColor clearColor];
    [self.dateView addSubview:self.datePicker];
    
    
    self.line4=[[UIView alloc] initWithFrame:CGRectMake(0, 220, self.dateView.size.width, 1)];
    ...
    self.cancelButton.frame=CGRectMake( (self.dateView.size.width/2-80)/2, 220+10, 80, 35);
    ...
    [self.cancelButton setBackgroundColor:RGBA(229, 228, 227, 1.0)];
    ...
    self.okButton.frame=CGRectMake((self.dateView.size.width/2-80)/2+self.dateView.size.width/2,220+10, 80, 35);
    [self.dateView addSubview:self.okButton];
    [self.okButton setBackgroundColor:RGBA(3, 163, 214, 1.0)];
...
-(void)defaultConfig {
    ...
    _yearArray = [TimeModel convertFromArray:_dateDic];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    if (component==0) {
        ...
        TimeModel * Timemodel = _yearArray[_selectYear];
        return Timemodel.time.count;
    }else{
    
        TimeModel * Timemodel = _yearArray[_selectYear];
        MinuteModel * model = Timemodel.time[_selectTime];
        return  model.min.count;
    }
}
-(CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component{
    if (component==0) {
        return self.datePicker.size.width*2/5;
    } else {
        return self.datePicker.size.width/5;
    }
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    NSString *title;
    if (component==0) {
      TimeModel * model = _yearArray[row];
      title = model.date;
    if (row==0) {
        _dateStr = title;
    }
            ...
            TimeModel * model = _yearArray[_selectYear];
            title =  model.time[row].hour;
    ...
    if (row==0) {
        ...
        TimeModel * model = _yearArray[_selectYear];
            ...
            MinuteModel * minModel = model.time[_selectTime];
            
            title = minModel.min[row];
    ...
    NSString *dateStr = [NSString stringWithFormat:@"%@ %@:%@",_dateStr,_hourStr,_minStr];
    
    _startDate = dateStr;
    self.chooseTimeLabel.text = dateStr;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    if (component == 0) {
       ...
       // ;
        //滑动第一列时改变第 2 ，3列的选择行
        if (_selectYear+1>_yearArray.count) {
           ...
           _selectTime = 0;
            ...
            TimeModel * model = _yearArray[_selectYear];
            _dateStr = model.date;
            _hourStr = model.time[0].hour;
            MinuteModel * minModel = model.time[0];
            _minStr = minModel.min[0];
        ...
        _selectTime = row;
        TimeModel * model = _yearArray[_selectYear];
        if (_selectTime+1>model.time.count) {
            
        }else{
            _hourStr =  model.time[row].hour;
            MinuteModel * minModel = model.time[_selectTime];
            _minStr = minModel.min[0];
        }
        ...
        TimeModel * model = _yearArray[_selectYear];
        MinuteModel * minModel = model.time[_selectTime];
        _minStr = minModel.min[row];
        
    }
    
    if ([_minStr isEqualToString:@"0"]) {
        _minStr = @"00";
    }else if([_minStr isEqualToString:@"5"]){
        _minStr = @"05";
    }
...
-(UIPickerView *)datePicker {
    if (!_datePicker) {
        _datePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(W(50), 20, self.dateView.size.width-W(100), 220)];
