...
- (instancetype) init {
    if (self = [super init]) {
        ...
        [self.tableView registerClass:[CNewsInfoCell class] forCellReuseIdentifier:@"CNewsInfoCell"];
        // 刷新header
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NewsInfoModel *model = _dataSource[indexPath.row];
    ...
    [cell isLastCell:(indexPath.row == (_dataSource.count - 1)) ? YES : NO];
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row < self.dataSource.count) {
        if (self.selectNewsInfoCellListener) {
            self.selectNewsInfoCellListener(self.dataSource[indexPath.row]);
        }
    }
}
...
- (void) refreshDataSource:(NSArray *)data nilDataTint:(NSString *)nilDataTint {
    ...
    if (_dataSource.count == 0) {
        [self addNilDataView];
        [promptView setNilDataWithImagePath:@"weizhaodao" tint:(nilDataTint ? nilDataTint : @"暂无新闻资讯") btnTitle:nil];
    } else {
        if (promptView) {
