...
NSString * const YZUpdateMenuTitleKey = @"YZUpdateMenuTitleKey";
...
- (void)setup {
    ...
    _observer = [[NSNotificationCenter defaultCenter] addObserverForName:YZUpdateMenuTitleNote object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        // 隐藏下拉菜单
        [self dismiss];
        if (col >= self.menuButtons.count) {
            return ;
        }
        // 获取对应按钮
        UIButton *btn = self.menuButtons[col];
        // 设置按钮标题
        [btn setTitle:note.userInfo[YZUpdateMenuTitleKey] forState:UIControlStateNormal];
        
        // 区域、道路 联动
        YZSortViewController *vc = (YZSortViewController *)note.object;
        if ([vc.ctxTag isEqualToString:@"areaSort"]) {// 选择区域，则初始化道路
            UIButton *btn = self.menuButtons[1];
            [btn setTitle:@"道路选择" forState:UIControlStateNormal];
        }
...
- (void)layoutSubviews {
    ...
    // 布局子控件
    float count = (float)self.menuButtons.count;
    
    CGFloat btnW = CTXScreenWidth / count;
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        if (i < count - 1) {
            ...
            separateLine.frame = CGRectMake((i+1) * btnW, _separateLineTopMargin,
                                            1, btnH - 2 * _separateLineTopMargin);
        }
    }
...
- (void) updateButton:(UIButton *)btn {
    ...
    // 文字的宽度
    CGFloat width = [KLCDTextHelper WidthForText:btn.titleLabel.text withFontSize:CTXTextFont withTextHeight:20];
    // 文字 图片，中间有3个间隔，每个间隔距离是4。14是图片的宽度
    CGFloat maxWidth = btnW - 14 - 4 * 3;
    width = width > maxWidth ? maxWidth : width;
    CGFloat left = ((btnW - 14 - 4) - width) / 2 - 14;
    
    // UIButton的titleEdgeInsets和imageEdgeInsets属性 设置
    btn.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    btn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
    // imageView在右 titleLabel在左
    [btn setTitleEdgeInsets:UIEdgeInsetsMake(0, left, 0, 12)];
    [btn setImageEdgeInsets:UIEdgeInsetsMake(0, (btnW - 18), 0, 0)];
}
